...
std::vector<entry> collumn::unique()
{
...
table::table(const table& r){
    ...
    for(auto& it:r.cols){
        push(it.first);
        cols[it.first].role = it.second.role;
        cols[it.first].type = it.second.type;
    }
    for(auto& it:r.rows){
        push_row(it.get_row());
        rows[r_n-1].set_name(it.name);
    }
}
...
table table::operator[](const std::vector<int>& rows_indexes){
    ...
    for(const auto& col:cols){
        ...
        result[col.first].type = cols[col.first].type;
        result[col.first].role = cols[col.first].role;
    }
    int i=0;
    for(auto row:rows_indexes){
        result.push_row(rows[row].get_row());
...
void table::pop(const std::string& collumn){
    if(cols.find(collumn)!=cols.end()){
        cols.erase(collumn);
        std::cout<<"brisem "<<collumn<<std::endl;
        c_n--;
        if(c_n<=0){
            c_n=0;
            r_n=0;
...
void table::auto_deduce_types()
{
    for(auto& it:cols){
        if(std::all_of(it.second.begin(),it.second.end(),[](entry x){return x.is_cast_valid();})){
            it.second.type=CONTINUOUS;
            for(auto& x :it.second.entries)
                x.to_double();
        } else
            it.second.type=NOMINAL;
    }
}
...
std::string table::get_target() const
{
    std::string t="no_target";
    for(const auto& it:cols)
        if(it.second.role==column_role::TARGET)
            t=it.first;
    return t;
}
